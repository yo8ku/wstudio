...
import { VSCodeThemeLoader, ITheme } from './vscode-adapter/ThemeLoader';
...
export class ThemeManager extends EventEmitter {
  ...
  async registerThemesFromExtension(extensionPath: string): Promise<number> {
    ...
    const themes = await this.themeLoader.loadAllThemes(extensionPath);
    let registeredCount = 0;
    
    for (const theme of themes) {
      if (this.themeLoader.validateTheme(theme)) {
        this.themes.set(theme.id, theme);
        registeredCount++;
        console.log(`[ThemeManager] 已注册主题: ${theme.name} (${theme.id})`);
      } else {
        console.warn(`[ThemeManager] 主题验证失败，跳过: ${theme.name}`);
      }
    }
  ...
  registerTheme(theme: ITheme): boolean {
    if (!this.themeLoader.validateTheme(theme)) {
      console.warn(`[ThemeManager] 主题验证失败: ${theme.name}`);
      return false;
    }
  ...
  getThemePreview(themeId: string) {
    ...
    return this.themeLoader.getThemePreview(theme);
  }
