...
export class VectorStore {
     ...
     * 添加文档到向量存储
     */
    async addDocuments(texts, metadatas, options = {}) {
     ...
     * 添加文件到向量存储
     */
    async addFile(filePath, content, options = {}) {
    ...
    async search(query, options = {}) {
        ...
        const { topK = 5, modelName, filterMetadata, } = options;
        const response = await this.bridge.request({
     ...
     * 根据元数据查询向量ID
     */
    async getIdsByMetadata(filterMetadata) {
        if (!this.bridge.isServiceReady()) {
            await this.initialize();
        }
        const response = await this.bridge.request({
     ...
     */
    async processFilePaths(filePaths, options = {}) {
        if (!this.bridge.isServiceReady()) {
            await this.initialize();
        }
        const { modelName, knowledgeBaseId, chunkSize = 1000, chunkOverlap = 200, strategy = 'recursive', } = options;
        try {
