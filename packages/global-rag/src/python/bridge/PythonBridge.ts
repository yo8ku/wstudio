...
interface ChildProcessModule {
  ...
  exec: (command: string, options?: any, callback?: (error: Error | null, stdout: string | Buffer, stderr: string | Buffer) => void) => any;
}
...
interface ChildProcess {
  ...
  on(event: 'exit', callback: (code: number | null, signal: NodeJS.Signals | null) => void): void;
...
async function getChildProcessModule(): Promise<ChildProcessModule> {
  ...
  return childProcessModule as ChildProcessModule;
}
...
async function execAsync(
  command: string,
  options: { timeout?: number; env?: NodeJS.ProcessEnv; cwd?: string; stdio?: 'pipe' | 'inherit' }
  ...
  return new Promise((resolve, reject) => {
    ...
    childProcess.exec(command, execOptions, (error: Error | null, stdout: string | Buffer, stderr: string | Buffer) => {
      if (error) {
        reject(error);
      } else {
...
export class PythonBridge {
  ...
  private stdoutBuffer: string = ''; // 用于累积 stdout 数据
  ...
  private dependencyCheckCache: { checked: boolean; timestamp: number } | null = null;
  private dependencyCheckInProgress: boolean = false;
   ...
   * 使用缓存机制，确保只检查一次
   */
  async checkAndInstallDependencies(): Promise<void> {
    // 如果正在检查，直接返回
    ...
    try {
      ...
      const cached = store.get(cacheKey) as { checked: boolean; timestamp: number } | undefined;
...
`;
      ...
      try {
        ...
        const verifyErrorOutput = verifyError instanceof Error && 'stderr' in verifyError 
          ? (verifyError as any).stderr?.toString() || ''
          : '';
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (!trimmedLine.startsWith('{')) {
              ...
              const isCommonWarning = 
                trimmedLine.includes('jq package not found') ||
            ...
            const response: PythonServiceResponse = JSON.parse(trimmedLine);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // 检查是否是文本内容导致的问题
          const params = (sanitizedReq as any).params;
          if (params) {
      ...
      if (process.env.NODE_ENV === 'development' || process.env.DEBUG === 'true') {
        console.debug('[PythonBridge] 发送请求:', {
          method: (sanitizedReq as any).method,
          paramsKeys: Object.keys((sanitizedReq as any).params || {}),
      ...
      // 写入请求（使用 UTF-8 编码）
      this.process.stdin.write(requestJson, 'utf8', (error: Error | null) => {
        if (error) {
   ...
   */
  private sanitizeRequestForJSON(req: PythonServiceRequest): PythonServiceRequest {
    ...
    return sanitizeValue(req) as PythonServiceRequest;
  }
   ...
   * 清理搜索结果中的文本数据，确保编码正确
   */
  private cleanSearchResults(results: unknown[]): unknown[] {
