 ...
 * 描述：
 * 1. 首先将文档切分成较大的父块（1000-2000字符），保留完整逻辑和上下文
 * 2. 将每个父块进一步切分成多个微小的子块，使用递归切分策略
 * 3. 建立父子关系，记录子块属于哪个父块
 * 4. 只对子块进行向量化，父块作为原始文本存储
 */
...
export interface ParentChunk {
  ...
  metadata: {
    ...
    /** 父块的标签列表（如果使用标签标识切分） */
    tags?: string[];
    /** 切分方式：'tag' 表示基于标签，'token' 表示基于 tokens */
    chunk_method?: 'tag' | 'token';
    [key: string]: unknown;
  };
}
...
export class ParentChildChunker {
  // 内置固定配置（不允许用户自定义）
  private static readonly PARENT_CHUNK_SIZE = 1500; // 父块大小（字符数）
  private static readonly PARENT_CHUNK_OVERLAP = 100; // 父块重叠大小（字符数，可选0或100）
