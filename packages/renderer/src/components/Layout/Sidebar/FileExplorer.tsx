...
export const FileExplorer: React.FC = () => {
        ...
        if (result.data && Array.isArray(result.data)) {
          ...
          if (result.data.length > 0) {
            ...
            result.data.slice(0, 3).forEach((node, index) => {
              console.log(`  [${index}] ${node.name} (${node.type || (node.isDirectory ? 'directory' : 'file')}) - ${node.path}`);
            });
          }
  ...
  const handleFileClick = async (node: FileTreeNode) => {
    console.log('Node clicked:', node.path, 'isDirectory:', node.isDirectory);
        ...
        if (result?.success && result.data) {
          ...
          window.dispatchEvent(new CustomEvent('open-file', {
            detail: {
              ...
              language: result.data.language || 'plaintext',
  ...
  const insertCreatingNode = (
    ...
    if (normalizedTarget === normalizedWorkspace || !targetPath) {
      ...
      if (type === 'file') {
        // 找到第一个文件的索引
        const firstFileIndex = nodes.findIndex(child => !child.isDirectory);
    ...
    return nodes.map(node => {
      ...
      if (normalizedNodePath === normalizedTarget && node.isDirectory) {
