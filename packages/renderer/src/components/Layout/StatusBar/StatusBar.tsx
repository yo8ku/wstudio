...
export const StatusBar: React.FC<StatusBarProps> = () => {
  ...
  const [currentTabType, setCurrentTabType] = useState<'file' | 'settings' | 'ai-config' | 'markdown-preview' | 'knowledge' | null>('file');

  // 调试：监控状态变量
  useEffect(() => {
    console.log('[StatusBar] showBackgroundSettings 状态变量', showBackgroundSettings);
  ...
  useEffect(() => {
    ...
    console.log('[StatusBar] 当前 showBackgroundSettings:', showBackgroundSettings);

    // 监听显示背景图片设置面板的事件
    const handleShowBackgroundSettings = (event: any, ...args: any[]) => {
      console.log('[StatusBar] ========== 收到显示背景图片设置面板事件 ==========');
      ...
      console.log('[StatusBar] 当前 showBackgroundSettings 状态', showBackgroundSettings);
    ...
    ipcRenderer.on('background-image:show-settings', handleShowBackgroundSettings);
    console.log('[StatusBar] 已监听background-image:show-settings 事件');
    console.log('[StatusBar] 监听器函数', handleShowBackgroundSettings);
    ...
    return () => {
      ...
      ipcRenderer.removeListener('background-image:show-settings', handleShowBackgroundSettings);
    };
  }, []);
  ...
  useEffect(() => {
    const handleTabLanguageChange = (event: Event) => {
      ...
      if (customEvent.detail?.language) {
        ...
        const displayName = languageMap[customEvent.detail.language] || customEvent.detail.language.toUpperCase();
  ...
  useEffect(() => {
    const handleActiveTabChanged = (event: Event) => {
      const customEvent = event as CustomEvent<{
        tabType: 'file' | 'settings' | 'ai-config' | 'markdown-preview' | 'knowledge' | null;
  ...
  React.useEffect(() => {
    ...
    console.log('[StatusBar] 详情:', JSON.stringify(pluginStatusBarItems, null, 2));
    console.log('[StatusBar] ==============================');
  ...
  return (
    <div className="status-bar">
          ...
          .map(item => {
            ...
            return (
              <button
                ...
                onClick={() => item.command && executePluginCommand(item.command)}
      ...
      <div className="status-bar-right">
        ...
        {/* 通知中心 - 始终显示 */}
        <button className="status-bar-icon-btn" title="通知中心">
