/**
 * Layout component exports.
 */

export { MainLayout } from './MainLayout';
export { ActivityBar } from './ActivityBar';
export { StatusBar } from './StatusBar/StatusBar';
export { AIChatPanel } from './AIChatPanel/AIChatPanel';
export { MenuBar } from './MenuBar';

export { Sidebar } from './Sidebar/Sidebar';
export type { SidebarProps } from './Sidebar/Sidebar';
export { FileExplorer } from './Sidebar/FileExplorer/FileExplorer';
export { Search } from './Sidebar/Search/Search';
export { SourceControl } from './Sidebar/SourceControl/SourceControl';
export { Settings } from './Sidebar/Settings/Settings';

export { EditorArea } from './EditorArea';
export { TabBar } from './EditorArea';
export type { TabBarProps } from './EditorArea';
export { Breadcrumb } from './EditorArea';
export { EditorGroup } from './EditorArea';
export { MonacoEditor } from './EditorArea';

export type { EditorTab } from './EditorArea/EditorArea/EditorArea';
export type { ActivityBarItem } from './ActivityBar/ActivityBar';
