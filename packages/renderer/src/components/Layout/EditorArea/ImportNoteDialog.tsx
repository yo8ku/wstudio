...
import { FileIcon as AppFileIcon } from '../../Explorer/FileTree/FileIcons';
...
export const ImportNoteDialog: React.FC<ImportNoteDialogProps> = ({
  ...
  // 已勾选的文件夹路径 Set（记录哪些文件夹被勾选，用于智能层级勾选）
    ...
    const loadNotes = async () => {
      try {
        ...
        if (result?.success && result.data) {
          console.log('[ImportNoteDialog] 获取到笔记文件:', result.data.length);
          ...
          if (knowledgeId) {
            const existingFileNames = knowledgeBaseService.getExistingFileNames(knowledgeId);
            filteredNoteFiles = noteFiles.filter(note => !existingFileNames.has(note.title));
            console.log('[ImportNoteDialog] 过滤前:', noteFiles.length, '个文件，过滤后:', filteredNoteFiles.length, '个文件');
          }
          
          setNotes(filteredNoteFiles);
        } else {
          console.log('[ImportNoteDialog] 未找到笔记文件');
          setNotes([]);
        }
      } catch (error) {
    ...
    if (node.type === 'file' && node.file) {
      ...
      return (
        ...
        >
          ...
          {/* 使用应用真实的文件图标系统 */}
          <AppFileIcon fileName={node.name} size={16} />
          <div className="file-content">
            <h4 style={{ color: 'var(--editor-fg)' }}>{node.name}</h4>
          </div>
        </div>
      );
          ...
          >
            ...
            >
              ...
              <AppFileIcon 
                ...
                size={16} 
              />
              <span className="folder-name" style={{ color: 'var(--editor-fg)' }}>
                {node.name}
              </span>
            </div>
          </div>
  ...
  return (
    <div className="import-note-dialog-overlay">
      ...
      >
        ...
        >
          <h3 style={{ color: 'var(--editor-fg)', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24">
              <g fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {filteredNotes.length > 0 && (
              <div className="sort-buttons">
                <button
                  ...
                  style={{
                    color: sortType === 'fileType' ? 'var(--button-bg)' : 'var(--editor-fg)',
                    backgroundColor: 'transparent',
                  }}
                  title="按文件类型排序"
                >
                ...
                <button
                  ...
                  style={{
                    color: sortType === 'createdTime' ? 'var(--button-bg)' : 'var(--editor-fg)',
                    backgroundColor: 'transparent',
                  }}
                  title="按创建时间排序"
                >
