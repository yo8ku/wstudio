...
export const TabBar: React.FC<TabBarProps> = ({
  ...
  console.log('[TabBar v2.0] Rendering with', tabs.length, 'tabs', { hoveredTabId, activeTabId });
  ...
  const handleOpenSettingsJson = async () => {
    try {
      ...
      const jsonContent = result?.success && result.data 
        ? JSON.stringify(result.data, null, 2)
        : '{}';
      
      window.dispatchEvent(new CustomEvent('open-settings-json', {
        detail: { content: jsonContent }
      }));
    } catch (error) {
