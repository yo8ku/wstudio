...
// 添加 WebGPU 测试函数（在控制台调用 testWebGPU()）
// 直接在渲染进程主线程测试，不使用 Worker
(window as unknown as { testWebGPU: () => Promise<void> }).testWebGPU = async () => {
  console.log('[WebGPU Test] ========== 启动 WebGPU 向量化测试 ==========');
  
  try {
    // 检查 WebGPU 支持
    console.log('[WebGPU Test] 检测 WebGPU 支持...');
    let hasWebGPU = false;
    if (navigator.gpu) {
      console.log('[WebGPU Test] navigator.gpu 存在');
      const adapter = await navigator.gpu.requestAdapter();
      if (adapter) {
        const info = await adapter.requestAdapterInfo();
        console.log(`[WebGPU Test] WebGPU 适配器: ${info.vendor} - ${info.architecture}`);
        hasWebGPU = true;
      } else {
        console.log('[WebGPU Test] WebGPU 适配器不可用');
      }
    } else {
      console.log('[WebGPU Test] 浏览器不支持 WebGPU');
    }
    
    // 获取应用路径
    console.log('[WebGPU Test] 获取模型路径...');
    const result = await window.electron?.ipcRenderer.invoke('app:get-app-path');
    if (!result?.success || !result.data) {
      console.error('[WebGPU Test] 获取应用路径失败:', result?.error);
      return;
    }
    
    const modelPath = `${result.data}/resources/models`;
    console.log('[WebGPU Test] 模型路径:', modelPath);
    
    // 动态导入 transformers
    console.log('[WebGPU Test] 导入 @huggingface/transformers...');
    ...
    console.log(`[WebGPU Test] 使用设备: ${device}`);
    
    // 创建 pipeline
    console.log('[WebGPU Test] 加载模型: bge-small-zh-v1.5...');
    ...
    console.log(`[WebGPU Test] 模型加载完成 (${loadTime}ms)`);
    
    // 测试向量化
    console.log('[WebGPU Test] 开始测试向量化...');
    const testTexts = [
      '这是一个测试文本',
      '人工智能正在改变世界',
    ...
    for (let i = 0; i < testTexts.length; i++) {
      ...
      console.log(`  [${i + 1}] "${text.substring(0, 15)}..." - ${embedTime}ms (${vector.length}维)`);
    }
    
    const avgTime = Math.round(totalTime / testTexts.length);
    console.log('[WebGPU Test] ========== 测试完成 ==========');
    console.log(`设备: ${device}`);
    console.log(`总耗时: ${totalTime}ms`);
    console.log(`平均耗时: ${avgTime}ms/文本`);
    
  } catch (error) {
    console.error('[WebGPU Test] 测试失败:', error);
  }
};

console.log('[App] WebGPU 测试函数已注册，在控制台输入 testWebGPU() 运行测试');
