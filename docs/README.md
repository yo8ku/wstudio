# Note Studio 文档

欢迎来到 Note Studio 文档中心！

## 📖 目录导航

### 用户指南

- [快速开始](./guide/getting-started.md) - 快速上手指南
- [功能介绍](./guide/features/) - 详细功能说明
  - [编辑器](./guide/features/editor.md)
  - [知识库](./guide/features/knowledge-base.md)
  - [AI 助手](./guide/features/ai-assistant.md)
  - [主题系统](./guide/features/themes.md)
- [快捷键](./keyboard-shortcuts-guide.md) - 键盘快捷键列表

### 开发文档

- [开发环境搭建](./development/getting-started.md) - 如何开始开发
- [项目架构](./development/architecture.md) - 架构设计说明
- [项目结构](./development/project-structure.md) - 目录结构说明
- [插件开发](../packages/plugin-system/QUICK_START.md) - 如何开发插件
- [贡献指南](./development/contributing.md) - 如何贡献代码
- [代码规范](./development/code-style.md) - 编码规范

### API 文档

- [API 索引](./api/README.md) - API 文档总览
- [核心 API](./api/core-api.md) - 核心功能 API
- [UI 组件](./api/ui-components.md) - UI 组件库
- [知识库 API](./api/knowledge-base-api.md) - 知识库系统 API

### 其他

- [系统设置](./settings-system-guide.md) - 系统设置指南

##  快速链接

- [GitHub 仓库](https://github.com/yo8ku/WiseAI-Note-Studio)
- [问题反馈](https://github.com/yo8ku/WiseAI-Note-Studio/issues)
- [版本历史](../CHANGELOG.md)

##  文档贡献

如果你发现文档有误或需要改进，欢迎提交 PR！

---

**Note Studio** - 一个支持插件扩展的现代化笔记应用
