...
function readInteractionMode(value: RuntimeJsonValue | undefined): 'default' | 'editorSuggest' {
...
export function mountPluginSurface(context: PluginRuntimeSurfaceContext): () => void {
  ...
  let selectedButtons: HTMLButtonElement[] = [];

  const setKeyboardModeEnabled = (enabled: boolean): void => {
  ...
  const renderInstructions = (nextInstructions: readonly RuntimeInstructionSnapshot[]): void => {
  ...
  const renderItems = (nextSuggestions: readonly string[]): void => {
