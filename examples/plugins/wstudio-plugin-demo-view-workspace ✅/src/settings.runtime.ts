...
interface RuntimeSettingTabSummary {
  readonly id: string;
...
interface PluginRuntimeSurfaceContext {
  ...
  readonly host: {
    readonly showNotice: (payload: {
      readonly message: string;
      readonly level: 'success' | 'error' | 'warning' | 'info';
    }) => Promise<void>;
    readonly data: {
      readonly load: () => Promise<RuntimeJsonValue | null>;
      readonly save: (data: RuntimeJsonValue | null) => Promise<void>;
      readonly delete: () => Promise<void>;
    };
    readonly settings: {
      readonly getTabs: () => Promise<readonly RuntimeSettingTabSummary[]>;
    };
  };
...
export function mountPluginSurface(context: PluginRuntimeSurfaceContext): () => void {
  ...
  const refreshPersistedData = async (): Promise<void> => {
